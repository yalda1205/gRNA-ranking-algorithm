...
# Calculate the normalized indel ratio
normalized_indel_ratio = []
for spacer, indel_ratio  in zip(spacer_seq, percent_rank):
    indel_ratio_ontarget = library_df_filtered_on_target[f'indel_ratio_MV{MV}'][library_df_filtered_on_target['spacer']== spacer].mean()
      
    if indel_ratio_ontarget != 0:
        normalized_indel_ratio.append(indel_ratio/indel_ratio_ontarget)
    else:
        normalized_indel_ratio.append(indel_ratio)
...
plt.savefig('/mnt/singlefs-4/sandbox_yamidi/QC_Cellecta_data/ROC_offtarget_normalized_indel_feature1.png', bbox_inches='tight')

#%%
# Save the model to a file
joblib.dump(lr_final, '/mnt/singlefs-4/sandbox_yamidi/QC_Cellecta_data/offtarget_normalized_indel_feature_1.joblib')

#%%
coef_final = ((best_model.coef_).tolist()[0])
df_features = pd.DataFrame({'Feature_names': features_names, 'Feature_Value': coef_final})
df_features.to_csv(f'/mnt/singlefs-4/sandbox_yamidi/QC_Cellecta_data/selected_features1_MV{MV}_offtarget_normalized_indel_LR.csv', index=False)
